  ...
  the OCaml compiler must interpret it as Syntax.name only. "name" is basically a shortform for Syntax.name.**)
    ...
    *)
    ...
    and instr =
      ...
      | IDiv                            (** To perform the division operation*)
      | IAdd                            (** addition *)
      | ISub                            (** subtraction *)
      | IEqual                          (** equality *)
      | ILess                           (** less than *)
      ...
      | IHandle of frame                (** handle raised exception *)
      | IPushExn of string              (** push exception value to stack *)
      | IString of string               (* push string onto stack *)
      | ITryWith of string *frame       (* Try block with exception handling *)
    ...
    (** Exceptions indicating a runtime error *)
    exception Machine_error of string
    exception DivisionByZero of string (* for division by zero *)
    exception GenericException of string (* for generic exception *)
    ...
    let string_of_mvalue = function
      ...
      | MExn msg-> "Exception: " ^ msg
      | MString str -> str (* Handle string value*)  
    ...
    (** Division: pops two integers, checks for division by zero, and performs division. *)
    let div = function
    | (MInt x) :: (MInt y) :: s ->
        if x = 0 then
          (* Push the exception onto the stack and raise it *)
         MExn "Division by zero" :: s (* Will be raised by IRaise *)
        else MInt (y / x) :: s
    | _ -> error "non zero int expected in divide"
    ...
    let exec instr frms stck envs =
      ...
      (match envs with
        ...
        (*This code is part of a virtual machine that executes instructions by manipulating three things: 
        stck (the stack for temporary values like strings or exceptions), 
        frms (the frame stack for tracking execution contexts like handlers), 
        and envs (the environment for variables, not used here). 
        Instructions like IPushExn msg and IString str add values to the stack. 
        IRaise looks for an exception on top of the stack and continues with it. 
        IHandle checks if there is an exception to catch and, if so, runs a new handler frame. 
        ITryWith (msg, frame) matches a specific exception 
        and activates its handler frame only if the message matches.*)
         | IRaise -> 
          (* Raise the exception  by pushing it to the stack *)
      (match stck with
           | MExn msg :: rest ->
          (* Continue with exception value on top, to be handled by IHandle *)
            (frms, MExn msg :: rest, envs)
            | _ -> error "IRaise expects an exception on top of the stack")
    
          | IHandle handler_frame ->
        (* Handle raised exception, pop exception and execute handler *)
      (match stck with
           | MExn _ :: _-> 
             (* Found an exception: drop it, run the handler *)
            (handler_frame :: frms, [], envs)
            | _ ->  (frms, stck, envs))    (* No exception, continue as normal *)
     
          | IPushExn msg -> (frms, MExn msg :: stck, envs) (* This pushes an exception value onto the stack, 
          like MExn "some message". *)
          | IString str -> (frms, MString str :: stck, envs)   (* Handle IString instruction *)        
          | ITryWith (expected_msg, handler_frame) ->
        (match stck with
            MExn actual_msg :: rest when actual_msg = expected_msg ->
             (handler_frame :: frms, rest, envs)
                 | _ -> (frms, stck, envs))  (* no match, continue as normal *)
